function [u, x, t] = T5bExplicitoNat(alpha, cc1, cc2, ci, L, nx, Tmax, nt)
    ...
    lambda = k*alpha^2/(h^2*8*pi^2) % Ojo
    ...
    for j=1:nt
        u(1, j+1) = (1-2*lambda+12*h)*u(1,j) + 2*lambda *u(2,j) - 2*h*lambda*c1t(1);
        u(2:nx, j+1) = (1-2*lambda)*u(2:nx,j) + lambda*(u(3:nx+1,j)+u(1:nx-1,j));
        u(nx+1, j+1) = (1-2*lambda+h*lambda)*u(nx+1,j) + 2*lambda*u(nx, j) + 2*h*lambda*c2t(nx+1);
    end
end
