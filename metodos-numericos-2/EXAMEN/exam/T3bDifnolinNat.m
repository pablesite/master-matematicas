function [X, Y, iter, incr ] = T3bDifnolinNat(f, fy, fy_p, a, b, alfa, beta, N, maxiter, tol)
    ...
    while incr >tol && iter < maxiter
        ...
        y_p = [alfa-Y(1) y_p (Y(end)+beta)]; % CAMBIAR
        ...
        dp(end) = 2*(1-h)+h^2*fye(end)+h^2*fy_pe(end);
        ...
        d(N+2) = -2*y(N+1)+(2-2*h)*y(N+2)+h^2*fe(N+2)-2*beta*h;
        d(N+2) = -d(N+2);
