function [u, x, t] = ExplicitoNat(alpha, cc1, cc2, ci, L, nx, Tmax, nt)
    ...
    for j=1:nt
        u(1, j+1) = (1-2*lambda+2*h*lambda)*u(1,j) + 2*lambda *u(2,j) - 2*h*lambda*c1t(1)+2;
        u(2:nx, j+1) = (1-2*lambda)*u(2:nx,j) + lambda*(u(3:nx+1,j)+u(1:nx-1,j)) + 2;
        u(nx+1, j+1) = (1-2*lambda-2*h*lambda)*u(nx+1,j) + 2*lambda*u(nx, j) - 2*h*lambda*c2t(nx+1) + 2;
    end
end
